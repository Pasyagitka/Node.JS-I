...
http.createServer(function (request, response) {
    ...
    if (request.method == 'POST') {
        ...
        if (url.parse(request.url).pathname == '/09-04') {
            let data ='';
        ...
        if (url.parse(request.url).pathname == '/09-05') {
            let data ='';
            request.on('data', (chunk) => { data += chunk; });
            request.on('end', () => {
        ...
        if (url.parse(request.url).pathname == '/09-06'){
            let form = new mp.Form({uploadDir:'./static'});
            form.on('file', (name, file) => { console.log(name, file); });
